...
#                 pred_binary = (pred_stack >0.99).float()    #( * (1-entropy_maps[0])
...
#                     pred_without_overlap = (pred[0]>0.99) * invert_overlap_map[0]
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        for iter, data in enumerate(train_dataloader):
            ...
            for j in range(0, len(gt_masks[0]), slice_step):
                ...
                pred_binary = ((pred_stack[0]>0.95)* ((1- entropy_maps)>0.75) ).float()   #* ((1- entropy_maps)>0.75)
                overlap_count = pred_binary.sum(dim=0)
                overlap_map = (overlap_count > 1).float()
                invert_overlap_map = 1.0 - overlap_map
